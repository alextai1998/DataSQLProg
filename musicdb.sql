...
CREATE TABLE Played (
  ...
  PRIMARY KEY (Played, Track_id, Artist_id, Album_id),
  FOREIGN KEY (Artist_id) REFERENCES Artist (Artist_id),
  FOREIGN KEY (Album_id) REFERENCES Album (Album_id),
...
SELECT * FROM Album WHERE (Album_name LIKE "L%" OR Album_name LIKE "S%") AND Album_name LIKE "%g";
...
SELECT * FROM Artist WHERE (Artist_name LIKE "%es" OR Artist_name LIKE "The%") AND NOT (Artist_name LIKE "%es" AND Artist_name LIKE "The%");
SELECT Artist_id, Album_id FROM Album WHERE Album_name LIKE "Brotherhood";
SELECT Track_name FROM Track WHERE Album_id = 7 AND Artist_id = 1;

# EXERCISES 2
SELECT Track_name, time FROM Track WHERE time < 5 LIMIT 3;
SELECT Time, Track_name, Artist_id FROM Track ORDER BY time DESC LIMIT 5;
